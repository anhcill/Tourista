...
export default function AdminTourEditPage() {
  ...
  const loadTour = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const data = await adminApi.getTourById(tourId);
      if (!data) return;
      ...
      setDurationDays(data.durationDays ? String(data.durationDays) : '1');
      setMaxGroupSize(data.maxGroupSize ? String(data.maxGroupSize) : '15');
      setMinGroupSize(data.minGroupSize ? String(data.minGroupSize) : '1');
      ...
      if (Array.isArray(data.itinerary)) {
        setItinerary(data.itinerary.map((it: { id?: number; dayNumber: number; title: string; description?: string }) => ({
          id: it.id,
      ...
      if (Array.isArray(data.departures)) {
        setDepartures(data.departures.map((d: { id?: number; departureDate: string; availableSlots: number; priceOverride?: number }) => ({
          id: d.id,
      ...
      setError(err instanceof Error ? err.message : 'Khong the tai chi tiet tour.');
    } finally {
      setLoading(false);
    }
  }, [tourId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess('Cap nhat tour thanh cong.');
      setTimeout(() => router.push('/admin/tours'), 1500);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Cap nhat tour that bai.');
    } finally {
      setSubmitting(false);
    }
  };
        ...
        <form onSubmit={handleSubmit}>
          <div className={styles.formGrid}>
            ...
            <label className={styles.fullWidth}>
              ...
              <input value={reason} onChange={(e) => setReason(e.target.value)} placeholder="Ly do thay doi thong tin" />
            </label>
          </div>
